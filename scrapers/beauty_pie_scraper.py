#!/usr/bin/env python3
"""
Beauty Pie scraper for skincare products.
"""
...
class BeautyPieScraper(BaseScraper):
    def __init__(self, delay: float = 4.0):
        super().__init__(delay=delay, use_selenium=False)
        self.base_url = "https://www.beautypie.com"
        
    def search_products(self, category: str = "moisturizer", limit: int = 10) -> List[str]:
        """
        Search for skincare products on Beauty Pie.
        """
        self.logger.info(f"Starting Beauty Pie search for {category}, limit: {limit}")
        ...
        category_urls = {
            'moisturizer': [
                ...
                f"{self.base_url}/products/skincare/moisturizer"
            ],
            'cleanser': [
                f"{self.base_url}/skincare/cleansers",
                f"{self.base_url}/skincare/face-wash",
                f"{self.base_url}/products/skincare/cleanser"
            ]
        }
        ...
        try:
                ...
                if soup:
                    ...
                    for link in product_links:
                        ...
                        if any(pattern in href for pattern in [
                            ...
                            if full_url not in product_urls and 'beautypie.com' in full_url:
            ...
            self.logger.error(f"Error searching Beauty Pie: {e}")
    ...
    def scrape_product_details(self, url: str) -> Optional[Dict]:
        """
        Scrape details from a Beauty Pie product page.
        """
        try:
    ...
    def scrape_products(self, category: str = "moisturizer", limit: int = 10) -> List[Dict]:
        """
        Scrape products from Beauty Pie.
        """
        self.logger.info(f"Starting Beauty Pie scraping for {category} category, limit: {limit}")
        ...
        self.logger.info(f"Successfully scraped {len(scraped_products)} products from Beauty Pie")
