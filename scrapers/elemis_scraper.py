#!/usr/bin/env python3
"""
Elemis scraper for skincare products.
"""
...
class ElemisScraper(BaseScraper):
    def __init__(self, delay: float = 4.0):
        super().__init__(delay=delay, use_selenium=False)
        self.base_url = "https://us.elemis.com"
        
    def search_products(self, category: str = "moisturizer", limit: int = 10) -> List[str]:
        """
        Search for skincare products on Elemis.
        """
        self.logger.info(f"Starting Elemis search for {category}, limit: {limit}")
        ...
        try:
                ...
                if soup:
                    ...
                    for link in product_links:
                        ...
                        if any(pattern in href for pattern in [
                            ...
                            if full_url not in product_urls and 'elemis.com' in full_url:
            ...
            self.logger.error(f"Error searching Elemis: {e}")
    ...
    def scrape_product_details(self, url: str) -> Optional[Dict]:
        """
        Scrape details from an Elemis product page.
        """
        try:
    ...
    def scrape_products(self, category: str = "moisturizer", limit: int = 10) -> List[Dict]:
        """
        Scrape products from Elemis.
        """
        self.logger.info(f"Starting Elemis scraping for {category} category, limit: {limit}")
        ...
        self.logger.info(f"Successfully scraped {len(scraped_products)} products from Elemis")
