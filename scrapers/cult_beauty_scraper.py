#!/usr/bin/env python3
"""
Cult Beauty scraper for skincare products.
"""
...
class CultBeautyScraper(BaseScraper):
    def __init__(self, delay: float = 4.0):
        super().__init__(delay=delay, use_selenium=False)
        self.base_url = "https://www.cultbeauty.com"
        
    def search_products(self, category: str = "moisturizer", limit: int = 10) -> List[str]:
        """
        Search for skincare products on Cult Beauty.
        """
        self.logger.info(f"Starting Cult Beauty search for {category}, limit: {limit}")
        ...
        category_urls = {
            'moisturizer': [
                f"{self.base_url}/skincare/moisturisers",
                f"{self.base_url}/skincare/face-moisturiser",
                f"{self.base_url}/skincare/creams"
            ],
            'cleanser': [
                f"{self.base_url}/skincare/cleansers",
                f"{self.base_url}/skincare/face-wash",
        ...
        try:
                ...
                if soup:
                    ...
                    for link in product_links:
                        ...
                        if any(pattern in href for pattern in [
                            ...
                            if full_url not in product_urls and 'cultbeauty.com' in full_url:
            ...
            # If no specific category pages work, try main skincare page
            if not product_urls:
                main_url = f"{self.base_url}/skincare"
            ...
            self.logger.error(f"Error searching Cult Beauty: {e}")
    ...
    def scrape_product_details(self, url: str) -> Optional[Dict]:
        """
        Scrape details from a Cult Beauty product page.
        """
        try:
    ...
    def scrape_products(self, category: str = "moisturizer", limit: int = 10) -> List[Dict]:
        """
        Scrape products from Cult Beauty.
        """
        self.logger.info(f"Starting Cult Beauty scraping for {category} category, limit: {limit}")
        ...
        self.logger.info(f"Successfully scraped {len(scraped_products)} products from Cult Beauty")
