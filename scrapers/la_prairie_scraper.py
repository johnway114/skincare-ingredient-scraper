#!/usr/bin/env python3
"""
La Prairie scraper for luxury skincare products.
"""
...
class LaPrairieScraper(BaseScraper):
    def __init__(self, delay: float = 4.0):
        super().__init__(delay=delay, use_selenium=False)
        self.base_url = "https://www.laprairie.com"
        
    def search_products(self, category: str = "moisturizer", limit: int = 10) -> List[str]:
        """
        Search for skincare products on La Prairie.
        """
        self.logger.info(f"Starting La Prairie search for {category}, limit: {limit}")
        ...
        try:
                ...
                if soup:
                    ...
                    for link in product_links:
                        ...
                        if any(pattern in href for pattern in [
                            ...
                            if full_url not in product_urls and 'laprairie.com' in full_url:
            ...
            self.logger.error(f"Error searching La Prairie: {e}")
    ...
    def scrape_product_details(self, url: str) -> Optional[Dict]:
        """
        Scrape details from a La Prairie product page.
        """
        try:
    ...
    def scrape_products(self, category: str = "moisturizer", limit: int = 10) -> List[Dict]:
        """
        Scrape products from La Prairie.
        """
        self.logger.info(f"Starting La Prairie scraping for {category} category, limit: {limit}")
        ...
        self.logger.info(f"Successfully scraped {len(scraped_products)} products from La Prairie")
